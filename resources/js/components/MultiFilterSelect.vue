...
<template>
    <SelectRoot v-model="selected" multiple>
        <SelectTrigger :id="id" class="flex h-9 w-full items-center justify-between gap-2 rounded-full px-3.5 text-left text-[13px] outline-none transition-colors hover:bg-neutral-200 focus-visible:ring-2 focus-visible:ring-ring/50 dark:hover:bg-neutral-700" :class="modelValue.length ? 'bg-neutral-200 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-100' : 'bg-muted text-foreground'">
            <span class="shrink-0 text-muted-foreground" aria-hidden="true">{{ label }}</span><span v-if="modelValue.length" class="min-w-0 flex-1 truncate font-medium">{{ modelValue.length === 1 ? modelValue[0] : `${modelValue.length} selected` }}</span><ChevronDownIcon class="size-3.5 shrink-0 text-muted-foreground" aria-hidden="true" />
        </SelectTrigger>
        <SelectPortal>
